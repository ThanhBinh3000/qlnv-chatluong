...
public class MoneyConvert {
	...
	public static final String PHAY = "phẩy ";
	public static final String LINH = "linh";
	public static final String VA = "và";
	public static final String XU = "xu";
	public static final String DONGF = "đồng";

	public static final String[] number = { KHONG, MOT, HAI, BA, BON, NAM, SAU, BAY, TAM, CHIN };

	public static String doctienBangChu(String sotien, String currCode) {
		// TODO: tam fix chi doc voi vnd
		if (StringUtils.isEmpty(currCode)) {
			currCode = DONGF;
		}
		return docTien(sotien, currCode);
	}

	public static String docTien(String number, String currCode) {
		String sotien = "";
		String arr[] = number.split(",");
		String phannguyen = arr[0];

		ArrayList<String> rs_phannguyen = readNum(phannguyen);

		String kq_phannguyen = "";
		String kq_phanthapphan = "";
		for (int i = 0; i < rs_phannguyen.size(); i++) {
			kq_phannguyen = kq_phannguyen + rs_phannguyen.get(i) + " ";
		}

		if (number.indexOf(",") > 0) {
			String phanthapphan = arr[1];
			phanthapphan = StringUtils.stripEnd(phanthapphan, "0");

			if (StringUtils.isNotBlank(phanthapphan)) {
				if (phanthapphan.length() <= 1) {
					phanthapphan += "0";
				} else {
					phanthapphan = StringUtils.stripStart(phanthapphan, "0");
				}
			}

			if (StringUtils.isBlank(phanthapphan)) {
				sotien = kq_phannguyen + currCode;
			} else {
				ArrayList<String> rs_phanthapphan = readNum(phanthapphan);
				for (int i = 0; i < rs_phanthapphan.size(); i++) {
					kq_phanthapphan = kq_phanthapphan + rs_phanthapphan.get(i) + " ";
				}
				sotien = kq_phannguyen + currCode + " " + VA + " " + kq_phanthapphan + XU;
			}
		} else {
			sotien = kq_phannguyen + currCode;
		}

		sotien = sotien.replace(VA + " " + LINH, VA + " " + KHONG);

		if (sotien.length() > 1) {
			sotien = sotien.substring(0, 1).toUpperCase() + sotien.substring(1);
		}
	...
	public static String docSoLuong(String number, String unit) {
		...
		if (number.indexOf(",") > 0) {
			...
			phanthapphan = StringUtils.stripEnd(phanthapphan, "0");
			if (StringUtils.isBlank(phanthapphan)) {
				sotien = kq_phannguyen + unit;
			} else {
				ArrayList<String> rs_phanthapphan = readNum(phanthapphan);
				for (int i = 0; i < rs_phanthapphan.size(); i++) {
					kq_phanthapphan = kq_phanthapphan + rs_phanthapphan.get(i) + " ";
				}
				sotien = kq_phannguyen + PHAY + kq_phanthapphan + unit;
			}
		} else {
			sotien = kq_phannguyen + unit;
		}

		sotien = sotien.replace(PHAY + LINH, PHAY + KHONG);
	...
	public static void main(String[] args) {
		String docSoLuong = docSoLuong("1007,08", "mét vuông");
		System.out.println("doc tien " + docSoLuong);
	}
}